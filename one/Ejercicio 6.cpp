...
class Token {
public:
    enum Type {PLUS, MINUS, MUL, DIV, PD, PI, NUM, SQRT, ERR, END}; // Se agrega SQRT
    Type type;
    string text;
    Token(Type);
    Token(Type, char c);
...
Token* Scanner::nextToken() {
    ...
    } else if (isalpha(c)) { // Reconocer palabras clave como sqrt
        current++;
        while (isalpha(input[current]))
            current++;
        string word = input.substr(first, current - first);
        if (word == "sqrt") {
            token = new Token(Token::SQRT, word, first, current - first);
        } else {
            token = new Token(Token::ERR, word, first, current - first);
        }
    } else if (strchr("+-*/()", c)) {
        switch (c) {
...
Exp* Parser::parseFactor() {
    ...
    if (match(Token::SQRT)) { // Manejo de sqrt
        match(Token::PI);
