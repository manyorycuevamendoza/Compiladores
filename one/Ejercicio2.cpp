...
class Token {
public:
    enum Type {PLUS, MINUS, NUM, ERR, END, MULT, DIV}; 
    Type type;
    string text;
    Token(Type);
    Token(Type, char c);
...
enum BinaryOp { PLUS, MINUS, MULT , DIV};
...
private:
    ...
    bool advance();
    bool isAtEnd();
    Exp* parseExpression(); //devuelve punteros expresiones
    Exp* parseTerm(); 
    Exp* parseFactor();
...
Token* Scanner::nextToken() { //lexico
    Token* token;
    while (input[current]==' ') current++; //ignora espacios
    if (input[current] == '\0') return new Token(Token::END); //fin de la cadena
    char c  = input[current]; //caracter actual
    first = current; //inicio del token
    if (isdigit(c)) { // si es un digito
        current++;
        while (isdigit(input[current])) //busca todo el numero entero
            current++;
        token = new Token(Token::NUM,input,first,current-first);  //lleva los contadores
    } else if (strchr("+-*/", c)) {
        switch(c) {
...
Exp* Parser::parse() {
    ...
    Exp* exp = parseExpression(); // parsea la expresion
...
Exp* Parser::parseExpression() { 
    ...
    while (match(Token::PLUS) || match(Token::MINUS) ||match(Token::MULT) || match(Token::DIV)){ //parse termine y si hay un + o - sigue parseando la expresion
        BinaryOp op;
        if (previous->type == Token::MINUS) {op = MINUS;}
        if (previous->type == Token::PLUS) {op = PLUS;}
        if (previous->type == Token::MULT) {op = MULT;}
