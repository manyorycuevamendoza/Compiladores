...
Token* Scanner::nextToken() {
    ...
    while (1) {
        switch (state) {
            ...
            case 10: rollBack();
                return new Token(Token::MULT,c);//devuelve solo 'c' porque no se ha leido el siguiente caracter, hablamos de  '*'
        }
    }
...
void Scanner::rollBack() {
    if (input[current] != '\0') // retrocede un caracter siempre y cuando no sea el final de la cadena
        current--;
}
